...
folder. It saves the sorted data in this format:
...
def dicominator(
    ...
    it can filter the files based on a given description and save the processed data in h5 format.
    ...
    Args:
        ...
        force (bool, optional): Force processing even if tags don't look like flow data.
    """
    ...
    patient_id = os.path.basename(os.path.normpath(output_root))
    target_folder = os.path.join(output_root, f"01_h5_input")
    ...
    if list_descriptions:
        ...
        if os.path.exists(target_folder):
            process_and_save_data(target_folder)
...
def process_and_save_data(output_root):
    """
    Process the sorted DICOM files and save the data in h5 format.
    ...
    #file_name = os.path.basename(output_root) #Original
    full_path = os.path.abspath(output_root)
    parent_folder = os.path.dirname(full_path)
    patient_id = os.path.basename(parent_folder)
    file_name = f"{patient_id}_LowRes"
    ...
    for file_path in tqdm(
        processed_files, desc=f"Processing {os.path.basename(output_root)}", ncols=100
    ):
    ...
    data = prepare_data_for_saving(
        image_data, venc_data, pos_pat, tt_pat, sample_ds, num_slices, num_cardiac_phases 
    )
...
def dicom_to_h5_serializable(val):
    """Converts a DICOM value to a type compatible with the h5py attributes."""
    
    def clean_string(s):
        """
        Cleans a string for h5py:
        1. Converts to string.
        2. Removes NULL characters (\x00).
        3. Ensures it is not an empty string (replaces with 'Empty').
        """
        ...
        if len(cleaned) == 0:
            return "Empty"  # h5py does not accept empty strings
        return cleaned

    # Handle complex types first
    if isinstance(val, pydicom.uid.UID):
        return clean_string(str(val))
    elif isinstance(val, pydicom.dataelem.PersonName):
        return clean_string(str(val))
    elif isinstance(val, pydicom.multival.MultiValue):
        # Recursively serialize items in the list
        serial_list = [dicom_to_h5_serializable(v) for v in val]
        # Convert the list to a string, since h5py doesn't handle lists in attrs well
        return clean_string(str(serial_list))
    elif isinstance(val, (bytes, bytearray)):
        try:
            decoded_val = val.decode('utf-8', 'ignore')
            return clean_string(decoded_val)
        except Exception:
            return "--- non-utf8-bytes ---"
    elif isinstance(val, pydicom.sequence.Sequence):
        return "--- DICOM Sequence (skipped) ---"
    elif isinstance(val, pydicom.dataset.Dataset):
        return "--- Nested Dataset (skipped) ---"
    
    if val is None:
        return "None"  # h5py does not accept None
    
    if isinstance(val, (int, float)):
        return val  # Numbers are fine as they are
    

    if isinstance(val, str):
        return clean_string(val)

    # Fallback for any other type (ej. DS, IS, etc.)
    try:
        if hasattr(val, 'value'):
            return dicom_to_h5_serializable(val.value)
        
        return clean_string(str(val))
    except Exception as e:
        logging.warning(f"Impossible to serialize type {type(val)}: {e}")
        return f"--- Unserializable Type: {type(val)} ---"
    

def dicom_to_dict(ds):
    """
    Converts a pydicom.Dataset into a simple dict to save as h5 attributes,
    USING AN ALLOW-LIST to save only relevant tags.
    """

    # --- IMPORTANT METADATA KEYS ---
    # Feel free to add or remove keywords here
    ...
    ALLOW_KEYWORDS = {
        ...
        # Physical Parameters (IMPORTANT!)
        "PixelSpacing",
        "SliceThickness",
        "RepetitionTime",
        "EchoTime",
        "FlipAngle",
        "NominalInterval",        # For Heart Rate
        "TriggerTime",
        "ImagePositionPatient",
        "ImageOrientationPatient",
        "FrameOfReferenceUID",
        "NumberOfTemporalPositions", # Important for 4D
        "RescaleSlope",
        "RescaleIntercept"
    }
    ...
    metadata_dict = {}
    for elem in ds:
        # Check if the tag has a keyword AND is in our allow-list
        if elem.keyword and elem.keyword in ALLOW_KEYWORDS:
            name = elem.keyword
            
            try:
                # Use the cleaning function defined above
                metadata_dict[name] = dicom_to_h5_serializable(elem.value)
            except Exception as e:
                logging.warning(f"Error serializing {name}: {e}")
            
    return metadata_dict


def prepare_data_for_saving(image_data, venc_data, pos_pat, tt_pat, sample_ds, num_slices, num_phases):
    """
    Prepare the data dictionary for saving.
    MODIFIED: Transpose the data to (T, x, y, z) format (for AI).
    ...
    """
    phaseRange = 4095
    for key in ["FH", "AP", "RL"]:
    ...
    tt_values = np.unique(tt_pat["MAG"])
    tt_sec = tt_values * 10**-3
    dt = np.mean(np.diff(tt_sec)) if len(tt_sec) > 1 else 0.0

    #---- DATA TRANSPOSITION TO (T, x, y, z) FORMAT ----
    rows, cols = image_data["MAG"].shape[0], image_data["MAG"].shape[1]
    formatted_data = {}
    mapping = {"RL": "u", "AP": "v", "FH": "w", "MAG": "mag"}

    for key, new_name in mapping.items():
        reshaped = image_data[key].reshape(rows, cols, num_slices, num_phases)
        transposed = np.transpose(reshaped, (3, 0, 1, 2))  # (T, x, y, z)

        if new_name == "mag":
            formatted_data["mag_u"] = transposed
            formatted_data["mag_v"] = transposed
            formatted_data["mag_w"] = transposed
        else:
            formatted_data[new_name] = transposed

    # Extract all metadata from a sample DICOM
    dicom_metadata = dicom_to_dict(sample_ds)

    data_dict = {
        "u": formatted_data["u"],
        "v": formatted_data["v"],
        "w": formatted_data["w"],
        "mag_u": formatted_data["mag_u"],
        "mag_v": formatted_data["mag_v"],
        "mag_w": formatted_data["mag_w"],
        "POS_PAT": pos_pat["MAG"],
        "TT_PAT": tt_pat["MAG"],
        "VENC": venc_data["FH"][0],
        "voxel_MR": [
            float(sample_ds.PixelSpacing[0]), # Ensure they are floats
            float(sample_ds.PixelSpacing[1]),
            float(sample_ds.SliceThickness),
        ],
        "heart_rate": round(60 / (float(sample_ds.NominalInterval) / 1000)),
        "type": "DCM",
        "dt": dt,
        "dicom_metadata": dicom_metadata  # Attach DICOM metadata
    }
    return data_dict
...
def save_h5_file(output_root, file_name, data):
    ...
    ...
    """
    keys_to_save = ["u", "v", "w", "mag_u", "mag_v", "mag_w", "voxel_MR"]

    # Pop metadata dictionary to handle separately as attributes
    dicom_metadata = data.pop('dicom_metadata', {})
    ...
    with h5py.File(os.path.join(output_root, f"{file_name}.h5"), "w") as f:
        ...
        if dicom_metadata:
            for key, value in dicom_metadata.items():
                try:
                    ...
                    # Fallback para cualquier tipo que haya fallado
                    try:
                        f.attrs[key] = str(value)
                    except Exception as e:
                         logging.warning(f"No se pudo guardar el atributo {key} (tipo {type(value)}): {e}")
        ...
        venc_val = data["VENC"]
        f.create_dataset("u_max", data=venc_val)
        f.create_dataset("v_max", data=venc_val)
        f.create_dataset("w_max", data=venc_val)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=__doc__,
        epilog=r"""Example: python dicominator.py /path/to/input /path/to/output --force""",
